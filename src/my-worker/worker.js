...
registerPromiseWorker(message => {
  // Check what type of computation we need to do
  // Set the threshold for the number of corners to detect

  const fastThreshold = message.message[0].selectedThreshold;
  console.log(fastThreshold);

  let imageData = message.message[1];
  ...
  for (let i = 0; i < edges.length; i++) {
    if (edges[i].colour > message.message[0].selectedThreshold) {
      centroids.push({
        x: edges[i].x,
        y: edges[i].y
      });
  ...
  const svg = d3
    ...
    .attr(
      "viewBox",
      `0 0 ${message.message[1].width} ${message.message[1].height}`
    )
  ...
  svg.on("click", () => {
    ...
    const colour = colourCentroidsByCoordinates(message.message[1], [
  ...
  const update = () => {
    ...
    const voronoi = delaunay.voronoi([
      ...
      message.message[1].height
    ]);

    // Construct the result
    if (message.message[0]["displayColour"]) {
      svg
        .selectAll("path")
    ...
    if (message.message[0]["displayEdges"]) {
      svg
        .selectAll("path")
        .style("stroke", "black")
    ...
    if (message.message[0]["displayCentroids"]) {
  ...
  update();
  // We need to do sobel edge detection
